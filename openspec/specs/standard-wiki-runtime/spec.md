...
Define the observable runtime behavior for a neutral wiki setup that follows `skill.io` conventions. This spec covers initialization inputs, the `WIKI.md` runtime contract, separation between configuration and data locations, and the canonical skill layout used by wiki workflows.
